...
export default async function edCdt(username, password) {
  if (typeof username !== "string" || typeof password !== "string") {
    ...
    if (responseLogin.data.code !== 200) {
      ...
      var config = {
        method: "post",
        url: `https://api.ecoledirecte.com/v3/Eleves/${accountId}/cahierdetexte.awp?verbe=get`,
        headers: {},
        data: data,
      };
...
export async function cdt(token, accountId) {
  var data = `data={\n\t"token": "${token}"\n}`;
  var config = {
    method: "post",
    url: `https://api.ecoledirecte.com/v3/Eleves/${accountId}/cahierdetexte.awp?verbe=get`,
